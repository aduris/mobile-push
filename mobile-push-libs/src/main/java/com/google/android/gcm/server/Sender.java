		...
		try {
			...
			try {
				...
				if (token.equals(TOKEN_MESSAGE_ID)) {
					...
					//2015-07-08 최일규 추가
					//성공일때, 디바이스토큰을 추가한다.
					builder.deviceToken(registrationId);
					...
					//토큰에러 일때, 디바이스토큰을 추가한다.
					return new Result.Builder().errorCode(value).deviceToken(registrationId).build();
				} else {
		...
		try {
			...
			if (results != null) {
				...
				for (Map<String, Object> jsonResult : results) {
					...
					String deviceToken = registrationIds.get(i);
